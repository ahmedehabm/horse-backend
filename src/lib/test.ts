...
async function createTestData(count: number) {
...
async function runRaceConditionTest(
  ...
  console.log(
    `   Total: ${count * 2} concurrent operations (${count} horses × 2)`,
  );

  const startTime = Date.now();

  // Create array of concurrent operations - SAME HORSE GETS BOTH
  const operations: Promise<TestResult>[] = [];
...
function analyzeResults(results: TestResult[]) {
  ...
  const scheduledFulfilled = fulfilled.filter(
    (f) => f.type === "scheduled",
  ).length;
  const scheduledRejected = rejected.filter(
    (f) => f.type === "scheduled",
  ).length;
  ...
  console.log(
    `  Scheduled: ${scheduledFulfilled} ✅ | ${scheduledRejected} ❌`,
  );

  // Check for race condition errors
  const raceConditionErrors = rejected.filter((r) =>
    r.error?.includes("already in progress"),
  );
  const otherErrors = rejected.filter(
    (r) => !r.error?.includes("already in progress"),
  );

  console.log(`\n🎯 Race Condition Handling:`);
  console.log(
    `  Correct rejections (already in progress): ${raceConditionErrors.length}`,
  );
  ...
  if (rejected.length === expected) {
    ...
    console.log(
      `  ✅ ${fulfilled.length} succeeded, ${rejected.length} rejected (as expected)`,
    );
    ...
    console.log(
      `  ❌ UNEXPECTED: ${rejected.length} rejections for ${fulfilled.length} successes`,
    );
  }
...
async function verifyDatabaseState(horseIds: string[]) {
